...
?>
	...
	alert("Customer Is Empty, Please recheck it");
	</script>
<?php
	}
...
//	if (empty($_POST))	
	...
	/* Check items 
	$sql = "select quantity from items where item_name = '".$items."';";
	$result = $db->query($sql);
	while($row = mysql_fetch_row($result)) {
	//	echo "Stock for ".$items." = ".$row[0]."<BR>";
	}; */
	...
	for($i=0;$i<sizeof($items);$i++){
		...
		$item_id_up = $items[$i];
		$qty_up = $itemsqty[$i];
		$sql2 = "select quantity from items where id = ".$items[$i].";";
		$result = $db->query($sql2);
		while($row = mysql_fetch_row($result)) {
			//echo "old = ".$row[0]."<BR>";
			//echo "up = ".$itemsqty[$i]."<BR>";
			$oldqty = $row[0];
			$qty_new = $oldqty - $itemsqty[$i];
			//echo "new = ".$qty_new."<BR>";
			$sql3 = "update items set quantity = ".$qty_new." where id = ".$item_id_up.";";
			$db->query($sql3);
		}
	}
...
<html>
<head>
  <title>My Valas Stock Management</title>
...
<tr>
  ...
  <table width="100%" height="100%" cellspacing="0">
	...
	<table><tr><td><B>Currencies</B></td></tr></table>
      <?php
	$sql = "select * from categories order by category ASC";
	$result = $db->query($sql);
	while($row = mysql_fetch_row($result)){
	?>
		<table class="tabcategory" onClick="javascript:loadXMLDoc('getItems.php?category=<?php echo $row[0]; ?>','getItems')"><tr><td><?php echo htmlspecialchars($row[1]); ?></td></tr></table>
	<?php
	}
	?>
      </div>
