...
if (isset($_POST['page_limit'])) {
	$_SESSION['pagenum'] = $_POST['page_limit'];
}

$pager = $_POST['page_limit'];
if (isset($_GET['nav'])) {
	$nav = $_GET['nav'];
	...
	case md5('prev'):
		...
		break;
	case md5('last'):
		$tots = $db->query("select count(*) from transaksibcu where status=1");
		$rows = mysql_fetch_row($tots);
		$_SESSION['pager'] = $rows[0].", 60";
		$append = $pager;
	}
	echo $append;
} else {
...
<script type="text/javascript">
  ...
  <?php
	$result = $db->query('select reseller.kode_reseller, 
		reseller.nama_reseller from reseller, 
		hp_reseller 
		where 
		reseller.kode_reseller = hp_reseller.kode_reseller 
		AND 
		reseller.mstatus=\'1\'');
  ...
  Jumlah Record: 
<input id="pg_lmt" type="text" size="3" value=<? if (isset($_POST['page_limit'])) {print $_POST['page_limit'];} else {print "30";}?> name="page_limit">
<input type="submit" value="Submit" name="submit_report1">

</form>
<?// if (isset($_POST['submit_report1'])) {?>
<center>
<a href="index.php?action=<?=md5('transaction')."&nav=".md5('beginning')."&".md5('page')."=".$_POST['pager']?>">paling awal</a>
<a href="index.php?action=<?=md5('transaction')."&nav=".md5('prev')."&".md5('page')."=".$_SESSION['pager']?>">sebelumnya</a> |
<a href="index.php?action=<?=md5('transaction')."&nav=".md5('next')."&".md5('page')."=".$_SESSION['pager']?>">selanjutnya</a>
<a href="index.php?action=<?=md5('transaction')."&nav=".md5('last')."&".md5('page')."=".$_SESSION['pager']?>">paling akhir</a>
</center>
<?// }?>
</div>
<?php
